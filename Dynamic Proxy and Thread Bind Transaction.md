...
헷갈렸던 부분은 쓰레드 바인딩된 커넥션을 반환하는 것이 아니고, 프록시를 통해 생성된 커넥션의 함수를 호출할 때마다 쓰레드에 바인딩된 커넥션을 동적으로 생성하고  Task를 수행 후 닫는 방식이라는 사실이었습니다.
...
주의할점은 프록시로 생성된 커넥션 객체가 쓰레드에 바인딩 된 것이아니고, 프록시를 통해 생성된 커낵션의 함수를 호출할때마다 dynamic proxy의 invoke메서드가 호출되면서 동적으로 쓰레드 바인딩된 커넥션을 만들고 Clinet의 Task를 수행 후 커넥션을 닫는다는 것입니다.
